/*
 * @Author       : wenwneyuyu
 * @Date         : 2024-05-22 14:09:36
 * @LastEditors  : wenwenyuyu
 * @LastEditTime : 2024-05-23 13:43:03
 * @FilePath     : /sylar/http/http_connection.cc
 * @Description  : 
    ...
    if (client_parser.chunked) {
      ...
      do {
        ...
        if (client_parser.content_len + 2 <= len) {
          ...
          while (left > 0) {
            int rt = read(data, left > static_cast<int>(buff_size)
...
HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string &uri,
  ...
  return std::make_shared<HttpConnectionPool>(turi->getHost(), vhost,
                                              turi->getPort(), max_size,
