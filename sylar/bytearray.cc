...
ByteArray::ByteArray(std::size_t base_size)
    : m_baseSize(base_size), m_position(0), m_capacity(base_size),
      m_size(base_size), m_endian(SYLAR_BIG_ENDIAN),
...
void ByteArray::write(const void *buf, size_t size) {
  ...
  while (size > 0) {
    if (ncap >= size) {
      memcpy(m_cur->ptr + npos, reinterpret_cast<const char *>(buf) + bpos,
             size);
      ...
      memcpy(m_cur->ptr + npos, reinterpret_cast<const char *>(buf) + bpos, ncap);
      m_position += ncap;
      bpos += ncap;
      size -= ncap;
      m_cur = m_cur->next;
