...
Socket::~Socket() {
  SYLAR_LOG_INFO(g_logger) << "~Socket()";
  close();
}
...
int Socket::send(const iovec *buffers, size_t length, int flags) {
  if (isConnected()) {
    ...
    msg.msg_flags = flags;
    return ::sendmsg(m_sock, const_cast<const struct msghdr *>(&msg), flags);
  }
  return -1;
}
...
int Socket::sendTo(const iovec *buffers, size_t length, const Address::ptr to,
  ...
  if (isConnected()) {
    ...
    msg.msg_iovlen = length;
    msg.msg_flags = flags;
    msg.msg_name = const_cast<void *>(static_cast<const void *>(to->getAddr()));
...
int Socket::recvFrom(iovec *buffers, size_t length, Address::ptr from,
  ...
  if (isConnected()) {
    ...
    msg.msg_iovlen = length;
    msg.msg_flags = flags;
    msg.msg_name = const_cast<void *>(static_cast<const void *>(from->getAddr()));
...
Address::ptr Socket::getLocalAddress() {
  ...
  if (getsockname(m_sock, const_cast<sockaddr *>(result->getAddr()),
                  &addrlen)) {
    SYLAR_LOG_ERROR(g_logger)
...
Address::ptr Socket::getRemoteAddress() {
  ...
  socklen_t addrlen = result->getAddrLen();
  if (getpeername(m_sock, const_cast<sockaddr *>(result->getAddr()),
                  &addrlen)) {
    SYLAR_LOG_ERROR(g_logger)
